...
public class SM2 {
	...
	//2w:170141183460469231731687303715884105728
	private static BigInteger _2w = new BigInteger("2").pow(w);
	//这是解密时候 确定密钥长度的参数 协商秘钥的时候未用到  int klen = encryptData.length - 65 - DIGEST_LENGTH;
	...
	public static SM2KeyPair generateKeyPair(BigInteger d) {
		SM2KeyPair keyPair = new SM2KeyPair(G.multiply(d).normalize(), d);
		
		System.out.println("私钥："+ Tools.BinaryToHexString(keyPair.getPrivateKey().toByteArray()));
		System.out.println("公钥: " + Tools.BinaryToHexString(keyPair.getPublicKey().getEncoded()));
		if (checkPublicKey(keyPair.getPublicKey())) {
			if (debug)
				System.out.println("generate key successfully");
			return keyPair;
...
//				//baos.write(params[i]);
//				baos.write(params[i],0,params[i].length);
//			}
...
//		return res;
		...
		int index = 0;
		for(int i=0;i<params.length;i++){
			System.arraycopy(params[i], 0, array,index, params[i].length);
...
//		byte[] idaBytes = IDA.getBytes();
		byte[] idaBytes = Tools.hexStringToBytes(IDA);
		System.out.println(new String(idaBytes)); new String(idaBytes);
		...
		System.out.println("gx:"+ Params.gx);
		System.out.println("gy:"+ Params.gy);
...
//				aPublicKey.getYCoord().toBigInteger().toByteArray());
		...
		byte[] ZA = sm3hash(ENTLA, idaBytes, Tools.hexStringToBytes(Params.a), Tools.hexStringToBytes(Params.b), Tools.hexStringToBytes(Params.gx), Tools.hexStringToBytes(Params.gy),
	...
	private static byte[] KDF(byte[] Z, int klen) {
		System.out.println("Z byte 大小： "+Z.length);
		String zToHexString = Tools.BinaryToHexString(Z);
		System.out.println("Z: "+ zToHexString);
		//截取前面的00
		String substring = zToHexString.substring(0, 2);
		if("00".equals(substring)){
			String substring2 = zToHexString.substring(2, zToHexString.length());
			Z = Tools.hexStringToBytes(substring2);
		}
...
//			this.ID=ID;
		 ...
		 * 接收发起方的公钥，临时公钥，ID
		 */
		public KeyExchange(String ID,byte[] publicKey){
//			this.ID=ID;
			this.Z=ZA(ID, curve.decodePoint(publicKey).normalize());
		}
		...
		public TransportEntity keyExchange_1() {
			rA = random(n);
			System.out.println("rA: "+rA.toString());
			...
			System.out.println("临时公钥：？  "+ Tools.BinaryToHexString(RA.getEncoded()));
			return new TransportEntity(RA.getEncoded(false), null,Z,keyPair.getPublicKey());
		}
		

		/**
		 * 密钥协商发起第一步 输入临时公钥 输入公钥
		 * 
		 * @return
		 */
		public TransportEntity keyExchange_1(byte[] tempPublicKey,byte[] publicKey) {
//			rA = random(n);
//			System.out.println("rA: "+rA.toString());
//			
//			// rA=new BigInteger("83A2C9C8 B96E5AF7 0BD480B4 72409A9A 327257F1
...
//			RA = G.multiply(rA).normalize();
//			System.out.println("临时公钥：？  "+ Tools.BinaryToHexString(RA.getEncoded()));
			return new TransportEntity(tempPublicKey, null,Z,curve.decodePoint(publicKey).normalize());
		}
...
//			BigInteger rB = random(n);
			...
			//b的临时公钥
			byte[] encoded = RB.getEncoded();
			System.out.println("b的临时公钥：" + Tools.bytesToHexString(encoded));
			...
			System.out.println("x2:"+Tools.BinaryToHexString(x2.toByteArray()));
			xyDelete0(x2);
			x2 = _2w.add(x2.and(_2w.subtract(BigInteger.ONE)));
			System.out.println("x2|:"+ Tools.BinaryToHexString(x2.toByteArray()));

			BigInteger tB = keyPair.getPrivateKey().add(x2.multiply(rB)).mod(n);
			System.out.println("tB:"+Tools.BinaryToHexString(tB.toByteArray()));
			...
			System.out.println("x1|:"+ Tools.BinaryToHexString(x1.toByteArray()));
			...
			//打印v的x，y
			System.out.println("xv:"+ Tools.BinaryToHexString(V.getXCoord().getEncoded()));
			System.out.println("xy:"+ Tools.BinaryToHexString(V.getYCoord().getEncoded()));
		 ...
		 * @return
		 */
		public byte[] keyExchange_2(TransportEntity entity,BigInteger rB) {
			//响应方的临时私钥
//			BigInteger rB = random(n);
//			BigInteger rB = new BigInteger("7E071248"+ "14B30948"+ "9125EAED" +"10111316"+ "4EBF0F34"+ "58C5BD88" +"335C1F9D"+ "596243D6", 16);
			// BigInteger rB=new BigInteger("33FE2194 0342161C 55619C4A 0C060293
			...
			byte[] yV = V.getYCoord().toBigInteger().toByteArray();
			System.out.println("yV:"+ Tools.BinaryToHexString(yV));
			System.out.println("xV:"+ Tools.BinaryToHexString(xV));
			
			if(xV.length==33){
				byte[] xV2 = new byte[32];
				System.arraycopy(xV, 1, xV2,0, 32);
				xV = xV2;
			}
			if(yV.length==33){
				byte[] yV2 = new byte[32];
				System.arraycopy(yV, 1, yV2, 0, 32);
				yV = yV2;
			}
			
			System.out.println("xV byte 大小 : "+ xV.length);
			System.out.println("yV byte 大小 : "+ yV.length);
			System.out.println("entity.Z 大小 : "+ entity.Z.length);
			System.out.println("Z 大小 : "+ this.Z.length);
			byte[] KB = KDF(join(xV, yV, entity.Z, this.Z), 16);
			key = KB;
			System.out.println("公钥:" + Tools.BinaryToHexString(key));
			return KB;
		}
		 ...
		 * @return
		 */
//		public static TransportEntity keyExchange_2_Temp(TransportEntity entity) {
//			BigInteger rB = random(n);
//			// BigInteger rB=new BigInteger("33FE2194 0342161C 55619C4A 0C060293
//			// D543C80A F19748CE 176D8347 7DE71C80".replace(" ", ""),16);
//			ECPoint RB = G.multiply(rB).normalize();
...
//			 
//		 
//			return new TransportEntity(RB.getEncoded(false), null,this.Z,keyPair.getPublicKey());
//		}
		
		public  void keyExchange_3_Temp(TransportEntity entity) {
			BigInteger x1 = RA.getXCoord().toBigInteger();
			x1 = _2w.add(x1.and(_2w.subtract(BigInteger.ONE)));
...
//		System.out.println("-----------------密钥协商-----------------");
		...
		//公钥双向转换
		ECPoint RA = curve.decodePoint(publicKeyA).normalize();
		byte[] encoded = RA.getEncoded();
		System.out.println("翻转后重新转回的公钥a " + Tools.bytesToHexString(encoded));
		
		TransportEntity entity3 = aKeyExchange.keyExchange_3(entity2);
		bKeyExchange.keyExchange_4(entity3);
		System.out.println("w:" + w);
		System.out.println("2w:" + _2w);
	}
...
//		
//		byte[] publicKey = Tools.hexStringToBytes(publicKeyHexStr);
//		byte[] tempPublicKey = Tools.hexStringToBytes(tempPublicHexStr);
//		
//		
//		
...
//	}
	 ...
	 * 去除坐byte标长度为33时前面的00
	 */
	public static BigInteger xyDelete0(BigInteger  coordinate){
		byte[] coordinateByte = coordinate.toByteArray();
		if(coordinateByte.length==33){
			String binaryToHexString = Tools.BinaryToHexString(coordinateByte);
			//去掉前面的00
			String newString = binaryToHexString.substring(2, binaryToHexString.length());
			BigInteger bigInteger = new BigInteger(newString, 16);
			String binaryToHexString2 = Tools.BinaryToHexString(bigInteger.toByteArray());
			System.out.println("转换后的字符串："+binaryToHexString2);
			return bigInteger;
		}else{
			return coordinate;
		}
